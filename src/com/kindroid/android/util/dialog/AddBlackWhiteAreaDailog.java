...
public class AddBlackWhiteAreaDailog extends Dialog implements
	 ...
	 * nc 必填选项
	 * */
	public AddBlackWhiteAreaDailog(Context context, int theme) {
		super(context, theme);
	}
	...
	public void onClick(View v) {
		...
		case R.id.sure_tv:
			...
			if (!nc.ismIsExists()) {
				...
				if (isRegionCode) {
					Toast.makeText(getContext(), R.string.area_code_existed,
							Toast.LENGTH_LONG).show();
				} else {
					Toast.makeText(getContext(), R.string.area_existed,
							Toast.LENGTH_LONG).show();
				}
				return;
			}
	...
	public void showDialog() {
		...
		if (nc != null) {
			mTitleTv.setText(getContext().getString(
					nc.getmRequestType() == 1 ? R.string.add_black_area
							: R.string.add_white_area));
		}

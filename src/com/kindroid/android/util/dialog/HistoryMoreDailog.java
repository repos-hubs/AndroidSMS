...
public class HistoryMoreDailog extends Dialog implements View.OnClickListener,
		OnItemClickListener {
	...
	private void findView() {
		...
		mGeneralDialog = new GenealDailog(getContext(), R.style.Theme_CustomDialog);
	 ...
	 * nc 必填选项
	 * */
	public HistoryMoreDailog(Context context, int theme,
	...
	public void showDialog() {
		this.show();
		MoreHistoryDialogAdapter adapter = null;
		...
		adapter = new MoreHistoryDialogAdapter(getContext(), hnc, stringId,
				null);
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		case 0:
			...
			SureDelHistoryDailog h_dialog=new SureDelHistoryDailog(getContext(), R.style.Theme_CustomDialog, mAdapter);
			h_dialog.setHnc(hnc);
			h_dialog.showDialog();
			...
			break;
		case 1:
			SureRestorySmsDailog res_dialog = new SureRestorySmsDailog(
					getContext(), R.style.Theme_CustomDialog, mAdapter);
			res_dialog.setHnc(hnc);
		...
		case 2:
			...
			if (!nc.ismIsExists()) {
				...
				Toast.makeText(getContext(), R.string.add_list_complete, Toast.LENGTH_LONG).show();
			}else{
				Toast.makeText(getContext(), R.string.number_already_exist, Toast.LENGTH_LONG).show();
			}
...
//						2, "", hnc.getmAddress(), true, true, 1);
		...
		case 6:
			...
			if ((telNoSms != null) && (!"".equals(telNoSms.trim()))) {
				...
				intent.setData(Uri.parse("sms:"+hnc.getmAddress()));
				getContext().startActivity(intent);
				
//				Intent intent=new Intent(getContext(),PrivacyDialogueDetailActivity.class);
//				intent.putExtra(PrivacyDialogueDetailActivity.ACTION_TYPE, PrivacyDialogueDetailActivity.ACTION_MMS_SEND);
//				intent.putExtra(PrivacyDialogueDetailActivity.ACTION_SOURCE_TYPE, PrivacyDialogueDetailActivity.ACTION_FORM_INTERCEPT);
//				intent.putExtra(PrivacyDialogueDetailActivity.ACTION_SOURCE_NUMBER, telNoSms);
//				intent.putExtra(PrivacyDialogueDetailActivity.ACTION_SOURCE_NAME, "");
//				getContext().startActivity(intent);
			}
			break;
		}
	...
	class AsyRestoreSms extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			InterceptDataBase.get(getContext()).insertBlackWhitList(
					nc.getmRequestType(), "", nc.getmPhoneNum(), true,
					true, 1);
			if (c != null) {
				c.close();
			}

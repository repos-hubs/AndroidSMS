...
public class SmsMessageSender {
	...
	public SmsMessageSender(Context context, String[] dests, String msgText, long threadId) {
		...
		splitMessage =
			mPrefs.getBoolean("",
//					mContext.getString(R.string.pref_split_message_key),
					DEFAULT_SPLIT_MESSAGE);
...
//								new Intent(SmsReceiverService.MESSAGE_SENT_ACTION, uri).setClass(mContext, SmsReceiver.class), 0);
					...
					smsManager.sendTextMessage(
							mDests[i], mServiceCenter, messages.get(j), 
//							sentIntent, 
							null,
							deliveryReportIntent);
				}
...
//											0));
	...
	private String getOutgoingServiceCenter(long threadId) {
		...
		try {
			cursor =
				mContext.getContentResolver().query(MessengerUtils.SMS_CONTENT_URI,
